...
📌 観測必須指標
- フォーム数: ${formCount}個
- CTA要素数: ${b2b?.ctaCount ?? 0}個
- alt属性なし画像数: ${seo.images.withoutAlt}個
- H1有無: ${seo.headings.h1 > 0 ? 'あり' : 'なし'}
`;
...
【重要】推測ではなく、上記の分析結果で観測できた事実と数値を根拠に述べてください。
【重要】本文中で必ず「フォーム数」「CTA要素数」「alt属性なし画像数」「H1有無」を引用してください。
【重要】改善点ごとに、どの観測値に基づく提案かが分かるように記述してください。
...
- B2B視点を必ず含める
- 推測ではなく観測値ベースで述べること
- フォーム数 / CTA要素数 / alt属性なし画像数 / H1有無 を本文中で必ず明記すること
- 余分な改行や空白を入れない
`;
    ...
    try {
      ...
      const response = await openai.chat.completions.create({
        ...
        messages: [
          {
            role: "system",
            content: "あなたは経験豊富なUX/UIデザイナーかつB2Bマーケティングの専門家です。回答は必ずHTMLフォーマットで提供してください。Markdown記法は使用しないでください。divタグは使わず、直接内容を出力してください。推測で補わず、入力として与えられた観測値と数値を根拠に説明してください。"
          },
          {
            role: "user",
            content: prompt
